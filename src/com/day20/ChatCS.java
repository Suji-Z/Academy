...
public class ChatCS extends Frame implements ActionListener, Runnable{
	...
	//DatagramSocket: UDP로 데이터를 전송하거나 수신	packet -> socket
	...
	public void setup() {
		...
		try {
			...
			xGroup = InetAddress.getByName(host);	//동일한 IP의 사람들끼리 묶음
	...
	public void actionPerformed(ActionEvent e) {
		...
		String msg = tf.getText().trim();	//공백 지우고 msg에 넣기
		if(msg.equals(null))
			return;
		
		byte[] buffer = (userName + "]" + msg).getBytes(); //String값인 ()을 byte로 변환
